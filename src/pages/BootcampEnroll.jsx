      ...
      if (Array.isArray(d) && d.length > 0) {
        ...
        if (token && user._id) {
          ...
          if (enrollments.includes(userId)) {
            ...
            navigate("/dashboard/bootcamp", { replace: true });
            return;
          }
  ...
  if (step === 3) return (
    <div className="min-h-screen bg-[#0B0F10] py-16 px-4 flex items-center justify-center">
      <div className="max-w-lg w-full">
        ...
        {authToken ? (
          <>
            <button onClick={() => navigate("/dashboard/bootcamp")} className="w-full bg-[#C7E36B] text-black font-bold py-3 rounded-xl hover:bg-lime-300 transition-all mb-3">
              GO TO MY BOOTCAMP →
            </button>
